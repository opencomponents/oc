...
|`forwardAcceptLanguageToClient`|`boolean`|no|Default false. When true, when doing a client-side requests (normal or failover) appends a custom parameter to the browser href so that the framework will ignore the browser Accept-Language in favour of the one specified via the client. This is needed when components that depend on the Accept-Language header are rendered during a failover and need to preserve the original server-side request's Accept-Language header|
...
|`forwardAcceptLanguageToClient`|`boolean`|no|When not specified in config, defaults to false. When true, when doing a client-side requests (normal or failover) appends a custom parameter to the browser href so that the framework will ignore the browser Accept-Language in favour of the one specified via the client. This is needed when components that depend on the Accept-Language header are rendered during a failover and need to preserve the original server-side request's Accept-Language header|
